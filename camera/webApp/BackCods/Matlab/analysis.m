function [Dstr] = analysis4valume()
    ...
    y80 = 100*(1-exp(-((D80./xc).^nc)));
    y50 = 100*(1-exp(-((D50./xc).^nc)));
    y40 = 100*(1-exp(-((D40./xc).^nc)));
    y20 = 100*(1-exp(-((D20./xc).^nc)));
    ...
    yb=100*(1-exp(-(((bbbb(:,2)/100)./xc).^nc)));
    figure,stairs(bbbb(:,1)*calibcoeff,yb);
    
    %yb=100*(1-exp(-(((bbbb(:,1)/100)./xc).^nc)));
    %figure,stairs(yb*calibcoeff,bbbb(:,2));
